 ...
 //peso_libras , precio_orden, cantidad_piezas, fecha_solicitud, fecha_entrega,
   ...
   while ($row=mysql_fetch_object($q)){
	...
	if(isset($_REQUEST['notify'])){
		$q=mysql_query("select c.cedula Cedula, o.id_orden Orden, DATE_FORMAT(fecha_asigna, '%d-%m-%Y') AS \"Fecha Asignado\",  uo.status  UsuarioOrdenS,uo.notify from orden_servicios o inner join clientes c on(o.id_cliente=c.reg_id)  inner join usuario_ordenes uo on(uo.id_orden=o.id_orden and uo.status IN('1','4')) where uo.id_usuario='$idc' and notify=0 and o.status IN('2','9')");
		while ($row=mysql_fetch_object($q)){
			if($row->Cedula!=NULL)
			$data[]=$row;
			$q="update usuario_ordenes set notify=1 where id_orden='".$row->Orden."' and id_usuario='$idc'";
			mysql_query($q);
   		}
	}
}
